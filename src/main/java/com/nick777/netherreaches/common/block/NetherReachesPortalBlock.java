...
import com.nick777.netherreaches.common.world.NetherReachesTeleporter;
...
public class NetherReachesPortalBlock extends Block {
    public static final EnumProperty<Direction.Axis> AXIS = BlockStateProperties.HORIZONTAL_AXIS;
    protected static final VoxelShape X_AABB = Block.makeCuboidShape(0.0D, 0.0D, 6.0D, 16.0D, 16.0D, 10.0D);
    protected static final VoxelShape Z_AABB = Block.makeCuboidShape(6.0D, 0.0D, 0.0D, 10.0D, 16.0D, 16.0D);

    public NetherReachesPortalBlock() {
    ...
    public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState facingState, IWorld worldIn, BlockPos currentPos, BlockPos facingPos) {
        ...
        return !flag && facingState.getBlock() != this && !(new NetherReachesPortalBlock.Size(worldIn, currentPos, directionAxis1)).canCreatePortal() ? Blocks.AIR.getDefaultState() : super.updatePostPlacement(stateIn, facing, facingState, worldIn, currentPos, facingPos);
    }
    ...
    public void changeDimension(ServerPlayerEntity entity, DimensionType destination) {
        if (!ForgeHooks.onTravelToDimension(entity, destination))
            return;
        entity.isInvulnerableDimensionChange();
        DimensionType dimensiontype = entity.dimension;
        ServerWorld serverworld = entity.server.func_71218_a(dimensiontype);
        entity.dimension = destination;
        ServerWorld serverdest = entity.server.func_71218_a(destination);
        NetherReachesTeleporter netherreachesTeleporter = new NetherReachesTeleporter(serverdest);
        ...
        if (!netherreachesTeleporter.placeInPortal(entity, yaw)) {
            netherreachesTeleporter.makePortal(entity);
            netherreachesTeleporter.placeInPortal(entity, yaw);
        }
        serverworld.getProfiler().endSection();
...
//            worldIn.addParticle(ModParticles.PORTAL.get(), d0, d1, d2, d3, d4, d5);
    ...
    public BlockPattern.PatternHelper createPatternHelper(IWorld worldIn, BlockPos pos) {
        ...
        NetherReachesPortalBlock.Size gaiaportalblock$size = new NetherReachesPortalBlock.Size(worldIn, pos, Direction.Axis.X);
        ...
        if (!gaiaportalblock$size.isValid()) {
            ...
            gaiaportalblock$size = new NetherReachesPortalBlock.Size(worldIn, pos, Direction.Axis.Z);
        }
        ...
        int calculatePortalHeight() {
            ...
            for (height = 0; height < 21; ++height) {
                for (int i = 0; i < width; ++i) {
                    ...
                    if (blockstate.getBlock() == NetherReachesBlocks.NETHER_REACHES_PORTAL) {
        ...
        void placePortalBlocks() {
            for (int i = 0; i < this.width; ++i) {
                ...
                for (int j = 0; j < height; ++j) {
                    world.setBlockState(blockpos.up(j), NetherReachesBlocks.NETHER_REACHES_PORTAL.getDefaultState().with(AXIS, axis), 2);
                }
            }
        }
