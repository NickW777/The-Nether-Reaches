...
public class NetherReachesOreFeature extends Feature<NetherReachesOreConfig> {
    public NetherReachesOreFeature(Function<Dynamic<?>, ? extends NetherReachesOreConfig> deserialize) {
        super(deserialize);
    }
    ...
    private boolean tryPlaceVein(IWorld world, Random random, NetherReachesOreConfig config, double startX, double endX, double startZ, double endZ, double startY, double endY, int minX, int minY, int minZ, int sizeXZ, int sizeY) {
        int placedBlocks = 0;
                            ...
                            for (int bz = minBZ; bz <= maxBZ; ++bz) {
                                ...
                                if (deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ < 1.0) {
                                    ...
                                    if (!mask.get(maskIndex)) {
                                        mask.set(maskIndex);
                                        pos.setPos(bx, by, bz);
                                        if (world.getBlockState(pos).isIn(NetherReachesTags.Blocks.CAN_HOLD_ORES)) {
