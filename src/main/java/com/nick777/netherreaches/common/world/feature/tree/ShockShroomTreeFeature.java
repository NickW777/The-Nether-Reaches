...
public class ShockShroomTreeFeature extends NetherReachTreeFeature {
    ...
    private static final BlockState LOG = NetherReachesBlocks.SHOCK_SHROOM_STEM.getDefaultState();
    private static final BlockState LEAVES = NetherReachesBlocks.SHOCK_SHROOM_CAP.getDefaultState();

    private static final int BRANCH_SPACING = 3;

    public ShockShroomTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> deserialize) {
        super(deserialize);
        this.setSapling((IPlantable) NetherReachesBlocks.SHOCK_SHROOM_SAPLING);
    }
            ...
            case WEST:
                ...
                for (int branch = 0; branch < branchCount; branch++) {
                    ...
                    switch (direction) {
                        ...
                        case SOUTH:
                            while (dir == null || dir == lastDirection) {
                                dir = ShockShroomTreeFeature.ZFACE[random.nextInt(ShockShroomTreeFeature.ZFACE.length)];
                            }
                            lastDirection = dir;
                        ...
                        case WEST:
                            while (dir == null || dir == lastDirection) {
                                dir = ShockShroomTreeFeature.XFACE[random.nextInt(ShockShroomTreeFeature.XFACE.length)];
                            }
                            lastDirection = dir;
