...
public class BlightShroomTreeFeature extends NetherReachTreeFeature {
    ...
    private static final BlockState LOG = NetherReachesBlocks.BLIGHT_SHROOM_STEM.getDefaultState();
    private static final BlockState LEAVES = NetherReachesBlocks.BLIGHT_SHROOM_CAP.getDefaultState();

    private static final int BRANCH_SPACING = 3;

    public BlightShroomTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> deserialize) {
        super(deserialize);
        this.setSapling((IPlantable) NetherReachesBlocks.BLIGHT_SHROOM_SAPLING);
    }
            ...
            case WEST:
                ...
                for (int branch = 0; branch < branchCount; branch++) {
                    ...
                    switch (direction) {
                        ...
                        case SOUTH:
                            while (dir == null || dir == lastDirection) {
                                dir = BlightShroomTreeFeature.ZFACE[random.nextInt(BlightShroomTreeFeature.ZFACE.length)];
                            }
                            lastDirection = dir;
                        ...
                        case WEST:
                            while (dir == null || dir == lastDirection) {
                                dir = BlightShroomTreeFeature.XFACE[random.nextInt(BlightShroomTreeFeature.XFACE.length)];
                            }
                            lastDirection = dir;
