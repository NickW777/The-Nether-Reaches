...
public class ToxicShroomTreeFeature extends NetherReachTreeFeature {
    ...
    private static final BlockState LOG = NetherReachesBlocks.TOXIC_SHROOM_STEM.getDefaultState();
    private static final BlockState LEAVES = NetherReachesBlocks.TOXIC_SHROOM_CAP.getDefaultState();

    private static final int BRANCH_SPACING = 3;

    public ToxicShroomTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> deserialize) {
        super(deserialize);
        this.setSapling((IPlantable) NetherReachesBlocks.TOXIC_SHROOM_SAPLING);
    }
            ...
            case WEST:
                ...
                for (int branch = 0; branch < branchCount; branch++) {
                    ...
                    switch (direction) {
                        ...
                        case SOUTH:
                            while (dir == null || dir == lastDirection) {
                                dir = ToxicShroomTreeFeature.ZFACE[random.nextInt(ToxicShroomTreeFeature.ZFACE.length)];
                            }
                            lastDirection = dir;
                        ...
                        case WEST:
                            while (dir == null || dir == lastDirection) {
                                dir = ToxicShroomTreeFeature.XFACE[random.nextInt(ToxicShroomTreeFeature.XFACE.length)];
                            }
                            lastDirection = dir;
