...
@OnlyIn(Dist.CLIENT)
public class NetherReachesSignTileEntityRenderer extends TileEntityRenderer<NetherReachesSignTileEntity> {
    private static final ResourceLocation SHADE_SHROOM_TEXTURE = new ResourceLocation(MODID,"textures/entity/sign/shade_shroom.png");
    private final SignModel model = new SignModel();

    @Override
    public void render(NetherReachesSignTileEntity tileEntityIn, double x, double y, double z, float partialTicks, int destroyStage) {
    ...
    public NetherReachesSignTileEntityRenderer() {
    }

    private ResourceLocation getTextureForSign (Block sign) {
        if (sign != NetherReachesBlocks.SHADE_SHROOM_SIGN && sign != NetherReachesBlocks.SHADE_SHROOM_WALL_SIGN) {
                return sign != Blocks.DARK_OAK_SIGN && sign != Blocks.DARK_OAK_WALL_SIGN ? SHADE_SHROOM_TEXTURE : SHADE_SHROOM_TEXTURE;
        } else {
            return SHADE_SHROOM_TEXTURE;
        }
    }
